...
export default function IsmServicesPage() {
  const [series, setSeries]   = useState(null);
  const [labels, setLabels]   = useState({});
  const [active, setActive]   = useState(new Set(["pmi", "new_orders", "business_activity"]));
  ...
  useEffect(() => {
    fetch(`${API}/api/ism/services/series`)
      .then((r) => r.json())
  ...
  useEffect(() => {
    ...
    fetch(`${API}/api/ism/services/rankings?component=${commentsComponent}`)
      .then((r) => r.json())
      .then((d) => {
  ...
  useEffect(() => {
    ...
    fetch(`${API}/api/ism/services/industry-history?industry=${encodeURIComponent(commentsIndustry)}&component=${commentsComponent}`)
      .then((r) => r.json())
  ...
  useEffect(() => {
    ...
    Promise.all(
      components.map((col) =>
        fetch(`${API}/api/ism/services/rankings?component=${col}`)
          .then((r) => r.json())
          .then((d) => [col, d])
  ...
  return (
    <PageShell>
      <PageHeader
        ...
        subtitle="Institute for Supply Management — Monthly Services PMI components"
      />
  ...
  if (!anyData) {
    return (
      <div style={{ color: "#6b7280", fontSize: 13 }}>
        No rankings data available. Import ISM Services reports and re-import to populate sector rankings.
      </div>
    );
  }
  ...
  if (industryList.length === 0) {
    return (
      <div style={{ color: "#6b7280", fontSize: 13, padding: "40px 0" }}>
        No rankings data available. Fetch the latest ISM Services report from the admin page first.
      </div>
    );
  }
      ...
      {!data ? (
        ...
        <div style={{ overflowX: "auto" }}>
          ...
          <div style={{ marginTop: 12, fontSize: 11, color: "#4b5563" }}>
            Comments available from the first imported report forward.
          </div>
        </div>
      )}
    </div>
...
function EmptyState() {
  return (
    <div style={{ background: "var(--bg-surface)", border: "1px solid var(--border)", borderRadius: "var(--radius-none)", padding: "32px 24px", textAlign: "center" }}>
      <div style={{ fontSize: 14, color: "#6b7280", marginBottom: 12 }}>No ISM Services data imported yet.</div>
      <div style={{ fontSize: 13, color: "#4b5563" }}>
        Go to <span style={{ color: "#3b82f6" }}>Admin → ISM Services Data Import</span> to load historical reports.
      </div>
    </div>
  );
}
