...
const REGIONS = [
  ...
  { value: "UK", label: "United Kingdom", soon: true },
  { value: "EU", label: "EU Region",      soon: true },
  { value: "DE", label: "Germany",        soon: true },
  { value: "JP", label: "Japan",          soon: true },
...
export default function GdpPage() {
  ...
  useEffect(() => {
    Promise.all([
      fetch(`${API}/api/gdp/series`).then((r) => r.json()),
      fetch(`${API}/api/gdp/market-correlation`).then((r) => r.json()),
    ])
  ...
  const nominal = series?.GDP;
  const real    = series?.GDPC1;
  ...
  return (
    <PageShell>
      ...
      {tab === "gdp" && nominal && real && (
        <>
          <Panel title="Nominal GDP vs. Real GDP ($B)">
            <LineChart
              dates={nominal.dates}
              datasets={[
                { dates: nominal.dates, data: nominal.values, borderColor: nominal.color, borderWidth: 2, label: nominal.label },
      ...
      {tab === "correlation" && correlation && (
        <>
          <Panel title="Real GDP YoY (right axis) vs. S&P 500 YoY (left axis)">
            <LineChart
              ...
              datasets={[
                ...
                { dates: correlation.dates, data: correlation.sp500_yoy, borderColor: "#3b82f6", borderWidth: 1.5, label: "S&P 500 YoY", priceScaleId: "left" },
              ]}
            />
          </Panel>

          <Panel title={`Rolling ${correlation.window_quarters / 4}Y Correlation — Real GDP YoY vs. S&P 500 YoY (by lag)`}>
            <p style={{ fontSize: 12, color: "#4b5563", marginBottom: 12 }}>
              Lag N = S&P 500 YoY from N quarters earlier, correlated against the current quarter's GDP YoY
            ...
            <LineChart
              ...
              datasets={Object.entries(correlation.correlation)
                ...
                .map(([lag, values]) => ({
                  dates: correlation.dates, data: values,
