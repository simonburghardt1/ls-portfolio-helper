...
class IsmSvcReport(Base):
    """One row per monthly ISM Services (Non-Manufacturing) report — headline component values."""
    __tablename__ = "ism_svc_report"

    date: Mapped[date] = mapped_column(Date, primary_key=True)          # YYYY-MM-01

    # 11 components (index value, e.g. 54.0)
    pmi:                  Mapped[float | None] = mapped_column(Float)
    business_activity:    Mapped[float | None] = mapped_column(Float)
    ...
    inventory_sentiment:  Mapped[float | None] = mapped_column(Float)

    source_url:  Mapped[str | None] = mapped_column(String(500))
    scraped_at:  Mapped[datetime | None] = mapped_column(DateTime(timezone=True))


class IsmSvcIndustryRank(Base):
    """
    ...
    """
    __tablename__ = "ism_svc_industry_rank"

    id:        Mapped[int] = mapped_column(Integer, primary_key=True, autoincrement=True)
    date:      Mapped[date] = mapped_column(Date, ForeignKey("ism_svc_report.date"), nullable=False)
    ...
    __table_args__ = (
        UniqueConstraint("date", "component", "industry", name="uq_ism_svc_rank"),
    )


class IsmSvcComment(Base):
    """One respondent quote per industry per monthly report (from 'WHAT RESPONDENTS ARE SAYING')."""
    __tablename__ = "ism_svc_comment"
    ...
    __table_args__ = (UniqueConstraint("date", "industry", name="uq_ism_svc_comment"),)
